...
onOpen, onOpened, onClose, onClosed, onBackButtonPress, onPositionChange, onOverlayPress, onLayout, }, ref) => {
    ...
    const renderWithHOC = (child) => {
        return react_native_gesture_handler_1.gestureHandlerRootHOC(() => child);
    };
    const renderReactModal = (child) => (React.createElement(react_native_1.Modal, Object.assign({}, reactModalProps, { supportedOrientations: ['landscape', 'portrait', 'portrait-upside-down'], onRequestClose: handleBackPress, hardwareAccelerated: USE_NATIVE_DRIVER, visible: isVisible, transparent: true }), renderWithHOC(child)));
    if (!isVisible) {
        return null;
    }
    if (withReactModal) {
